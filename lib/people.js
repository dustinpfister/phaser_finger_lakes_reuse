...
const get_pt = (person, subType ) => {
    subType = subType === undefined ? person.getData('subType') : subType;
...
PEOPLE_TYPES.worker = {
    ...
    setSubType: (people, scene, person ) => {
...
class People extends Phaser.Physics.Arcade.Group {
    ...
    constructor (config, pConfig) {
        ...
        this.spawnRate = pConfig.spawnRate;// === undefined ? 3000: pConfig.spawnRate;
    }
    ...
    spawnPerson (scene) {
        ...
        if( people.length < this.maxSize && t >= this.spawnRate ){
            ...
            const person = this.get({ x: 0, y: 0, texture: 'people_16_16', frame:0 }, pConfig );
            person.setData('type', this.getData('type') );
            const pt = get_pt(person, false);
            pt.setSubType(this, scene, person);
            const st = get_pt(person, person.getData('subType') ); 
