    ...
    noPath : function(mdc, md, people, scene, person, opt){
        ...
        if(ioi){
            ...
            if(d > opt.min_dist){
                ...
                //console.log('yes path set', d, opt.min_dist);
...
ACTIONS.worker_di_process = {
    ...
    init: function(mdc, md, people, scene, person, opt, delta) {
        ...
        //person.setData('spot_count', 5);
        person.say('I will now go process');
    },
    ...
    noPath: function(mdc, md, people, scene, person, opt){
        const oh = person.getData('onHand');
        const moh = person.getData('maxOnHand');
        //let c = person.getData('spot_count');
        ...
        if(oh.length > 0 && opt.count <= 0 ){
             person.say('so I drop now yes.');
             people.setAction(scene, mdc, md, person, 'drop', { count: 1 } );
        }
        
        //person.setData('spot_count', c);
...
TASKS.di = {
    ...
    update: (mdc, md, people, scene, person, opt, delta) => {
        ...
        if(md.index === 4 && oh.length === 0){
            ...
            if(spot){
                console.log('yes we have empty boxes');
                people.setAction(scene, mdc, md, person, 'pickup', { type:'empty', container: true, max_items: 1  } );
            }
