...
ACTIONS.worker_di_return = {
    init: (mdc, md, people, scene, person) => {
         ...
         person.say(person.name + ' I need to return to di now');
    ...
    noPath: (mdc, md, people, scene, person) => {
        ...
        if(md.index != 4){
            ...
            person.say(person.name + 'I must find a door to map 4' );
        
            let door = mdc.doorCheck(scene, person, md);
            if(!door){
                const pos1 = person.getTilePos();
                const pos2 = md.findDoorFrom(pos1.x, pos1.y, 4, false);
                if(pos2){
