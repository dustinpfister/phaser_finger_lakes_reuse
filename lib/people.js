...
ACTIONS.pickup = {
    ...
    noPath : function(mdc, md, people, scene, person, opt){
        ...
        if(!ioi && oh.length < moh){
            ...
            if(items.length === 0){
                console.log('No Items to Pickup!');
                person.setData('action_done', true);
...
TASKS.di = {
    ...
    update: (mdc, md, people, scene, person, opt, delta) => {
        ...
        if(md.index === 4){
            people.setAction(scene, mdc, md, person, 'pickup', { container: false } );
        }
        ...
        if(md.index === 4 && oh.length === moh ){
            people.setAction(scene, mdc, md, person, 'goto_map', { index: 1 } );
        }
        
        if(md.index != 4 && oh.length === moh && done ){
