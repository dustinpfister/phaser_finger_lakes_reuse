            ...
            if(person.action === 'di'){
                ...
                if(ioi){
                    ...
                    if(d <= 2){
                        ...
                        //person.onHandAction(scene, ioi, pos.x, pos.y);
                        people.onHandAction(scene, person, ioi, pos.x, pos.y);
                    }
                    
                    const oh = person.getData('onHand');
                    const moh = person.getData('maxOnHand');
                    if( oh.length >= moh ){                        
                        console.log('I have the stuff baby!');
...
class People extends Phaser.Physics.Arcade.Group {
    ...
    onHandAction (scene, person, item, tx, ty) {
        const people = this;
        const onHand = person.getData('onHand');
        const maxOnHand = person.getData('maxOnHand');
        const im = person.getData('itemMode');
