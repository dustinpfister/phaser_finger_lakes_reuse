...
ACTIONS.donation_goto_droplocation = {
    ...
    init: (mdc, md, people, scene, person) => {
        person.say('I need to find a place to drop what I have on hand');
    ...
    update: (mdc, md, people, scene, person) => {
        ...
        if(onHand.length > 0 && tPos.x === -1 && tPos.y === -1 ){
            ...
            if(tiles_di.length > 0){
                person.say('Ah yes this looks like a good place to put this.', 'SAY');      
...
ACTIONS.donation_drop = {
    ...
    update: (mdc, md, people, scene, person) => {
        ...
        if(onHand.length > 0 && cPos.x === tPos.x && cPos.y === tPos.y){
            ...
            person.say('well yes we make it here fine');
...
ACTIONS.customer_goto_exit = {
    ...
    init: (mdc, md, people, scene, person) => {
        ...
        person.say('My work here is done.');
...
TASKS.catatonic = {
    ...
    init: (mdc, md, people, scene, person) => {
        ...
        //people.setAction(scene, mdc, md, person, 'whatDo' );
...
TASKS.donate = {
    ...
    init: (mdc, md, people, scene, person) => {
        person.say('I am here to drop off some items.');
