    ...
    noPath: (mdc, md, people, scene, person) => {
        ...
        if(person != player){
            ...
            if(person.action === 'find_stock_spot'){
                ...
                person.setPath(scene, md, tile.x, tile.y);
                person.action = 'stock';
                console.log('well now this is weird');
            }
            if(person.action === 'di'){
                ...
                if( oh.length >= moh ){
                    ...
                    if(!door){
                        ...
                        //const doors = md.hardMapData.doors.filter((door)=>{ return door.to.mapNum === 1  });
                        //door = doors[ Math.floor(doors.length * Math.random() ) ];
                        //console.log( door.position );
                        console.log('I am not at a door, maybe I should find one?');
                        const pos1 = person.getTilePos();
                        const pos2 = md.findDoorFrom(pos1.x, pos1.y, 1, false);
                        
                        if(pos2){
                            console.log('found a door position, maybe I should go to it?');
    ...
    noPath: (mdc, md, people, scene, person) => {    
        ...
        if(onHand.length > 0 && cPos.x === tPos.x && cPos.y === tPos.y){
            ...
            while(i_item--){
                ...
                const pos = person.getTilePos();
                people.onHandAction(scene, person, item, md, pos.x, pos.y);
            }
