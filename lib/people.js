...
ACTIONS.whatDo = {
    ...
    init: (mdc, md, people, scene, person) => {
        scene.mp.push('I am catatonic!', 'SAY');
        person.setData('path', [] );
    },
...
ACTIONS.wonder = {
    ...
    init: (mdc, md, people, scene, person) => {
        scene.mp.push('Okay guess I will wonder around then.', 'SAY');
    },
...
TASKS.catatonic = {
    ...
    init: (mdc, md, people, scene, person) => {
        scene.mp.push('Oh no! I am catatonic!', 'SAY');
        //person.action = 'whatDo';
    },
    ...
    canSpawn : function (mdc, md, people, scene, now) {
        ...
        if(current_count < max && t >= people.spawnRate && md.index === 4 ){
            ...
            if(current_count < max){
                //scene.mp.push('Can Spawn New Customer: ', 'DEBUG');
                return true;
            
            }
            
            scene.mp.push('max: ' + max, 'DEBUG');
        ...
        if(current_count >= max && t >= people.spawnRate && md.index === 4 ){
             people.setData('lastSpawn', now);
             people.setSpawnRate( sr.min, sr.delta );
             //scene.mp.push('Can NOT Spawn New Customer max reached: ' + current_count + '/' + max, 'DEBUG');
        }
...
PEOPLE_TYPES.customer.shopper = {
    ...
    noPath: (mdc, md, people, scene, person) => {
        ...
        if(items.length === 0){
            //scene.mp.push('I will just wonder around then', 'SAY');
            const tile = md.getRandomWalkTo();
            person.setPath(scene, md, tile.x, tile.y);
        }
        if(!ioi && items.length > 0){
            ...
            //scene.mp.push('I see something.', 'SAY');
        }
        if(ioi){
...
PEOPLE_TYPES.customer.donator = {
    ...
    update: (mdc, md, people, scene, person) => {
        ...
        //scene.mp.push('What am I doing here', 'SAY'); 
    ...
    create: (mdc, md, people, scene, person) => {
        ...
        people.setTask(scene, md, mdc, person, 'catatonic');
    },
