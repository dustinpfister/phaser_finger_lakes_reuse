...
class Action {
    ...
    callFunc (type='init', md ){
        const func = this.def[type];
        if(func){
            func.call(this, this.mdc, md, this.people, this.scene, this.person, this.opt);
        }
    }
    
    init ( md ) { this.callFunc('init', md); }
    update ( md ) { this.callFunc('update', md); }
...
class People extends Phaser.Physics.Arcade.Group {
    ...
    setAction (scene, mdc, md, person, actionKey = 'whatDo', opt ) {
        const people = this;
