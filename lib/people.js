...
ACTIONS.wonder = {
    ...
    init: (mdc, md, people, scene, person) => {
        person.say(person.name + ': Okay guess I will wonder around then.');
    },
    ...
    noPath: (mdc, md, people, scene, person) => {
        ...
        person.say(person.name + ': yes I am getting a new path here');
...
ACTIONS.worker_di_idle = {
    ...
    update: (mdc, md, people, scene, person) => {
        ...
        if( person === player ){
            person.say(person.name + ': yes I am working di, but I am under player control.');
            person.setData('action_done', true);
        ...
        if( md.index != 4 && oh.length === 0 ){    
             person.say( person.name + ' I have empty hands, and I am not at map 4!' );
             person.setData('action_done', true);
             person.setData('action_result', 'empty_handed');
        }
        if( md.index === 4){    
             person.say( person.name + ' I am here now what?' );
             person.setData('action_done', true);
...
ACTIONS.worker_di_return = {
    init: (mdc, md, people, scene, person) => {
         ...
         // first thing is first. Get the current map index of the person
         let mi = md.index;
         
         // get all the doors in the current map
         
         // if there is a foor to map 4, slect that door as the point in a set path call
         
         // if there is NOT a door to map4 just select the a door to the lowest map number
         // from the current one (for now) as the set path call.
         
         // when the person gets to a door enter the door
    ...
    update: (mdc, md, people, scene, person) => {
        ...
        if(md.index === 4){
            person.setData('action_done', true);
            person.setData('action_result', 'at_di');
        }
        
        let door = md.getDoorAt(pos1.x, pos1.y);
        const path = person.getData('path');
        console.log(person.name + ' mi=' + md.index + ' pos=' + pos1.x + ',' + pos1.y + ' door? ' + !!door, ' pathLen=' + path.length);
        
        //let door = mdc.doorCheck(scene, person, md);
        
        if(md.index === 4 ){
            
            person.say('ah yes I have returned');
    ...
    noPath: (mdc, md, people, scene, person) => {
        ...
        //if(md.index != 4 ){
            ...
            if(!door || (door && md.index != 4)){
                ...
                const door_to4 = options.some(( mapNum )=>{ return mapNum === 4  });
                
                const to_map = door_to4 ? 4 : min;
                ...
                if(pos2){
                    ...
                    person.say(person.name + 'I am taking a door to map ' + to_map );
                }
                
                if(!pos2){
         
                    
                    person.say(person.name + 'for some reason this is happening' );
...
TASKS.di = {
    ...
    update: (mdc, md, people, scene, person) => {
        ...
        if( done ){
            ...
            if( person.action === 'worker_di_idle' && result === 'empty_handed'){
                people.setAction(scene, mdc, md, person, 'worker_di_return' );
            }
            
            if( person.action === 'worker_di_idle' && result === 'at_di_back'){
                people.setAction(scene, mdc, md, person, 'wonder' );
            }
            
            if( person.action === 'worker_di_return' && result === 'at_di'){
        ...
        //if( person.getData('action_done') && md.index != 4 && person.action != 'worker_di_items_drop'){
        //    people.setAction(scene, mdc, md, person, 'worker_di_return' );
        //}   
...
TASKS.donate = {
    ...
    update: (mdc, md, people, scene, person) => {
        ...
        if(person.getData('action_done') ){
            ...
            if(person.action === 'donation_goto_droplocation'){
                people.setAction(scene, mdc, md, person, 'donation_drop' );
                return;
            }
            
            if(person.action === 'donation_drop'){
                people.setAction(scene, mdc, md, person, 'customer_goto_exit' );
                return;
            }
            
            if(person.action === 'customer_goto_exit' ){
...
PEOPLE_TYPES.worker.employee = {
    ...
    create: (mdc, md, people, scene, person) => {
        ...
        people.setTask(scene, md, mdc, person, 'di');
        ...
        if(!md.hardMapData.customer.disabled){
            ...
            while(i_people--){
                ...
                if(spt && person != player){
                    ...
                    //let action = ACTIONS[person.action];
                    ...
                    person.pathProcessorCurve( scene, (scene, person) => {
                        ...
                        //log(person.name, person.getTilePos()  );
                        //console.log('okay yes this is called');
                        let door = mdc.doorCheck(scene, person, md);
                        console.log(person.getTilePos().y); 
                        console.log( 'path processor curve callback: ' );
                        console.log(door);
                    ...
                    if( !person.getData('action_done') ){
                        ACTIONS[person.action].update(mdc, md, people, scene, person);
                    }
                }                                
            }
        }
