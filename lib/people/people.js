import { Action, ACTIONS_DEFAULT } from "./action.js";
...
const NOOP = function(){};
const ACTIONS = Object.assign({}, ACTIONS_DEFAULT);
/********* **********
 1.0) TASKS 
********** **********/
const TASKS = {};
/********* **********
 1.1) TASKS - player_control
********** **********/
...
TASKS.di = {
    ...
    update: (mdc, md, people, scene, person, opt, delta) => {
        ...
        if(action.done && action.key == 'worker_di_idle'){
            ...
            if( action.result === 'idle_with_items' ){
                ...
                if(item.iType === 'Container'){
                    people.setAction(scene, mdc, md, person, 'worker_di_idle', { index: 1 } );
                }
            }
        }
        ...
        if( action.done && (action.key === 'pickup' || action.key === 'pickup_drop') ){
            //if( action.result === 'have_items' ) {
                people.setAction(scene, mdc, md, person, 'worker_di_idle', {   } );
            //}
            //if( action.result === 'no_items' ) {
            //}
        }
        ...
        // the worker di return only hand one possible result 'at_di'
...
/********* **********
 3.0) PEOPLE_TYPES ( customer, worker )
********** **********/    
const PEOPLE_TYPES = {}
