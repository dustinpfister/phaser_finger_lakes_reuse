...
ACTIONS.worker_di_recycle_empty = {
    ...
    init: function(mdc, md, people, scene, person, opt, delta) {
         ...
         /*
         onHand.forEach( (item) => {
             log( 'have :' + item.key + '  ' + item.drop_count );
         });
         */
         
         if(onHand.length === 0){
             person.say('I do not have anything to recycle!');
             //person.setData('action_done', true);
             //person.setData('action_result', 'nothing_on_hand');
             person.setDone('nothing_on_hand');
             log('yes a worker_di_recycle_empty action ended because they have nothing on hand');
             return;
         }
         if(bin){
             ...
             if(pos){
             person.say('okay yes there is a bin there');
...
TASKS.di = {
    ...
    update: (mdc, md, people, scene, person, opt, delta) => {
        ...
        if(action.key === 'pickup' && action.done && action.result ==='have_items' ){
            
            const item = oh[0];
            console.log(action.key, action.done, action.result, item.iType);
            if(item.iType === 'Container' && item.drop_count === 0 && item.contents.length === 0 ){
                people.setAction(scene, mdc, md, person, 'worker_di_recycle_empty', { } );
            }
        }
    }
...
TASKS.donate = {
    ...
    update: (mdc, md, people, scene, person) => {
        ...
        if( action.done ){
            ...
            if(action.done && action.key === 'customer_goto_exit' && action.result === 'at_exit' ){
                people.kill(person);
                return;
            }
        }  
...
class People extends Phaser.Physics.Arcade.Group {
    ...
    setTask (scene, mdc, md, person, taskKey = 'catatonic' ) {
        ...
        console.log( 'setting to task: ' + person.task );
