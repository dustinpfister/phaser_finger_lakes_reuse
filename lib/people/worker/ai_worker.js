...
import { ACTIONS as ACTIONS_WORKER_NEW } from "./action_worker.js";
const ACTIONS_WORKER = Object.assign({}, ACTIONS_WORKER_NEW);
...
TASKS_WORKER.default = {
    init: function (mdc, md, people, scene, person) { 
        ...
        log('**********DEFAULT WORKER TASK **********');
        log('name : ' + person.name);
        log('main task : ' + main_task);
        if(!main_task || main_task === 'default'){
            log('sense I do not have a main task, I should get one.');
