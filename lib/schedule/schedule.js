...
class TimeBar  {
    ...
    constructor ( opt = {} ) {
        ...
        if(tb.key === '__MISSING'){   
            this.texture = this.scene.textures.createCanvas( opt.textureKey , this.w, this.h);
            const img = this.img = this.scene.add.image( this.x, this.y, opt.textureKey);
            img.setScrollFactor(0, 0);
            img.depth = 10;
            img.alpha = 0.80;
        }
     ...
     /*
        this.img_bg = opt.scene.add.image( opt.x, opt.y, opt.texture, 'timebar_bg');
    ...
    draw () {
        ...
        ctx.fillStyle = 'rgba(0,64,16,0.8)';
        ctx.fillRect( 0, 0, canvas.width, canvas.height);
        
        ctx.fillStyle = 'white';
        ctx.font = '10px monospace';
        ctx.fillText( 'Money: ' + gs.money, 10, 10);
