...
class TimeBar  {
    ...
    constructor ( opt = {} ) {
        opt = Object.assign({}, { scene: null, w: 600, h: 75, x: 320, y: 45, max_te_sprites: 10 }, opt);
        this.w = opt.w;
        this.h = opt.h;
    ...
    update (delta=0) {
        ...
        while(i_te < len){
            ...
            if(!te.sprite && time_cd_min <= 1){
                te.sprite = tb.group_te.get( 0, 0 );
                te.sprite.setScale(1);
            }
            
            if(te.sprite && time_cd_min <= 1 && time_cd_min >= 0){
                te.sprite.x = tb.x + 32;
                te.sprite.y = tb.y + 32;
                //te.sprite.y = gt.y;
            }
