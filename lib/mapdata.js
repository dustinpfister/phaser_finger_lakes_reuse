...
class MapData {
    ...
    getRecycling() {
        const rbins = this.hardMapData.objects.containers.recycle_bin;
        ...
        if(rbins){
            ...
            return this.getItemsAtTile(rbins[0])[0];
        }
        return null;
    }
    ...
    /*
        ...
        while(i < limit){
            ...
            spot.y = pos.y + Math.round(Math.sin(rad) * ray);
            const tile = this.map.getTileAt(spot.x, spot.y);
            //if(spot.y < 0 || spot.y >= this.map.height || spot.x < 0 || spot.x >= this.map.width){
            if(!tile){
                i += 1;
                continue;
            }
            const items = this.getItemsAtTile( spot );
            if(findEmpty && items.length === 0){
                return spot;
            }
            if(!findEmpty && items.length >= 1){
                return spot;
    ...
    findEmptyDropSpot (pos, limit=10) {
         //return this.findItemAtSpot(pos, limit, true);
    ...
    /*
        ...
        while(i < limit){
            ...
            if(spot.y < 0 || spot.y >= this.map.height || spot.x < 0 || spot.x >= this.map.width){
                //log( spot.x, spot.y);
                //log( 0,0, this.map.height, this.map.width );
                //log('out of bounds! Continuing...');
                i += 1;
                continue;
