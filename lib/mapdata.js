...
class MapDataCollection {
   ...
   constructor (scene, opt) {
       ...
       let i_map = arr[0];
       const i_stop = arr.slice(arr.length - 1, arr.length)[0] + 1;
       console.log(i_map, i_stop);
       while(i_map < i_stop){
           console.log(i_map);
           const mapData = new MapData(scene, i_map, { player : opt.player });
