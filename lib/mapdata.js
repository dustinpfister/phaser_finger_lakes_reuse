...
class MapDataCollection {
   ...
   doorCheck (scene, person) {
        ...
        if(door){
            ...
            console.log('a person went threw a door!');
            const pType = person.getData('type');
            console.log('it is a ' + pType + ' type person.');
            console.log( md.index + ' ' + md[pType].children.size );
            console.log( nmd.index + ' ' + nmd[pType].children.size );
   ...
   setActiveMapByIndex (scene, index) {
       ...
       this.forAllMaps(scene, (scene, md, i)=>{
           ...
           if(bool){
               if(!player){
                   player = md.worker.spawnPerson(scene);
