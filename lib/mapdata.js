...
class MapData {
    ...
    constructor (scene, map_index=0, opt) {
        ...
        console.log(hmd.tilePalette.indices)
        const data = scene.cache.tilemap.get('map' + map_index).data.trim().split(/\n/g).map( (row, i_row) => {
            ...
            //console.log(n, parseInt(n));
                ...
                console.log()
                return hmd.tilePalette.indices[parseInt(n)][1];
            });    
        });
...
class MapDataCollection {
   ...
   doorCheck (scene, person) {
        ...
        if(door){
            ...
            console.log('switching to map' + door.to.mapNum);
   ...
   setActiveMapByIndex (scene, index) {
       ...
       console.log('yes, setActiveMapByIndex was called');
