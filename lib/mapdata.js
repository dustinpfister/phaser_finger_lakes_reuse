...
class MapDataCollection {
   ...
   doorCheck (scene, person) {
        ...
        if(door){
            ...
            this.setActiveMapByIndex(scene, nmd.index);
        }
    }
   ...
   update(scene){
       let player = scene.registry.get('player');
       const mdc = this;
       mdc.forAllMaps(scene, function(scene, md, map_index){
           const worker = md.worker.spawnPerson(scene);
           if(!player && mdc.activeIndex === map_index){
           
           console.log('player created at map index ' + map_index)
