...
class Item extends BaseItem {
    ...
    constructor (scene, key, x=0, y=0) {
        const data = getItemData(scene, key, 'items_index' );
...
class Container extends BaseItem {
    ...
    constructor (scene, key="blue_bin", x=-1, y=-1) {
        const data = getItemData(scene, key, 'items_index' );
        super(scene, key, x, y, data.tile.sheet, data.tile.frame);
    ...
    spawnItem (scene) {
        ...
        if( drop_count === 0 && conLen > 0){
            ...
            this.setFrame(this.prefix + '_stuff');
            const data = getItemData(scene, itemRec.key );        
        ...
        if( drop_count > 0 ){
            ...
            this.setFrame(this.prefix + '_stuff');
            const data = getItemData(scene, 'hh_mug_1');
...
class ItemCollection extends Phaser.GameObjects.Group {
    ...
    getEmpties (canRecyle = true) {
        return this.getChildren().filter( (item) => {
            return item.canGet && canRecyle === item.canRecyle && item.drop_count <= 0 && item.contents.length === 0;
        }); 
    }
}
